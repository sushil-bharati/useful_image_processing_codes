#Modified from http://www.pyimagesearch.com/2014/03/24/building-pokedex-python-scraping-pokemon-sprites-step-2-6/

# We’ll use BeautifulSoup to parse our HTML and requests to download the Pokemon images.
#  Finally, argparse is used to parse our command line arguments.
...
for name in names:
	...
	url1 = "https://img.pokemondb.net/artwork/%s.jpg" % (parsedName)
	r = requests.get(url1)
	...
	f = open("%s/%s.jpg" % (args["sprites"], name.lower()), "wb")
	f.write(r.content)
	f.close()
